...
namespace MCDA_Scoring_System.MCDA_Scoring_System.Core.Services.WeightingServices
{
    public class PercentageAllocationService : IPercentageAllocationService
    {
        ...
        public async Task SetWeightsAsync(int decisionId, List<CriterionPercentageAllocationDto> weights)
        {
            ...
            if (decision.WeightingMethod != WeightingMethod.PercentageAllocation)            
                throw new ArgumentException(
                    "This decision does not use Percentage Allocation.");

            if (weights == null || weights.Count == 0)
                throw new ArgumentException("No criterion weights provided.");
            ...
            if (weights.Select(w => w.CriterionId).Distinct().Count()
                != weights.Count)
            {
                throw new ArgumentException(
            ...
            if (weights.Any(w => !criterionIds.Contains(w.CriterionId)))
            {
                throw new ArgumentException(
            ...
            var weightsByCriterion = weights.ToDictionary(w => w.CriterionId);

            foreach (var criterion in decision.Criteria)
            {
                var input = weightsByCriterion[criterion.Id];

                criterion.Weight = input.Weight / 100m;
            }
