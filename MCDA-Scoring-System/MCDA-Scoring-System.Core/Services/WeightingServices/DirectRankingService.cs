...
namespace MCDA_Scoring_System.MCDA_Scoring_System.Core.Services.WeightingServices
{
    public class DirectRankingService:IDirectRankingService
    {
        ...
        public async Task SetWeightsAsync(int decisionId, List<CriterionDirectRankingDto> rankings)
        {
            var decision = await _repo.All<Decision>()
            ...
            if (rankings.Select(r => r.CriterionId).Distinct().Count() != rankings.Count)
                throw new ArgumentException(
            ...
            if (rankings.Any(r => !criterionIds.Contains(r.CriterionId)))
                throw new ArgumentException(
                    "One or more criteria do not belong to this decision.");
            if (rankings.Select(r => r.Rank).Distinct().Count() != rankings.Count)
                throw new ArgumentException(
            ...
            var totalScore = numberOfCriteria * (numberOfCriteria + 1) / 2m;
            var rankingsByCriterion = rankings.ToDictionary(r => r.CriterionId);

            foreach (var criterion in decision.Criteria)
            {
                var ranking = rankingsByCriterion[criterion.Id];

                var score = numberOfCriteria - ranking.Rank + 1;

                criterion.Weight = score / totalScore;
            }
